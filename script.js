...
const state = {
  ...
  profile: { coins: 0, xp: 0, level: 1, winStreak: 0, trophies: 0 },
  matchHistory: [],
...
function initPoints(){
  TEAM_NAMES.forEach(team => {
    state.points[team] = { team, played:0, won:0, lost:0, points:0, runsFor:0, ballsFor:0, runsAgainst:0, ballsAgainst:0, nrr:0 };
  });
}
...
function renderProfile(){
  const xpCurrent = state.profile.xp % 100;
  els.profileLevel.textContent = `Level ${state.profile.level}`;
  ...
  els.xpFill.style.width = `${xpCurrent}%`;
}

function renderSquad(){
  const squad = SQUADS[state.userTeam];
...
function autoPickXI(){
  ...
  state.userXI = [...bat.slice(0,4).map(x=>x.idx), ...ar.slice(0,3).map(x=>x.idx), ...bowl.slice(0,4).map(x=>x.idx)].slice(0,11);
  renderSquad();
  renderPlayingXI();
}
...
function beginLiveMatch(){
  ...
  const battingOrder = state.userXI.map(idx => {
    const p = SQUADS[state.userTeam][idx];
    return {
      ...p,
      runs:0,
      balls:0,
...
function closeCurrentPartnership(finalClose = false){
  ...
  m.partnerships.push({ batters:m.currentPartnership.batters, runs:m.currentPartnership.runs, balls:m.currentPartnership.balls, final:finalClose });
...
function updateProbabilities(){
  ...
  if(striker.battingType === "aggressor"){
    probs[6] += 2; probs[4] += 1; probs["W"] += 1;
  } else {
    probs[1] += 2; probs[2] += 1; probs[6] -= 1;
  }

  if(striker.preferredAgainst === bowler.bowlingType){
    probs[4] += 2; probs[6] += 1; probs[0] -= 1;
  } else {
    probs[0] += 2; probs["W"] += 1;
  }
  ...
  if(m.inningsMode === "chase"){
    if(rrr > 7.5){ probs[4] += 3; probs[6] += 3; probs[1] -= 3; probs[0] += 1; }
    if(rrr > 9){ probs[6] += 2; probs["W"] += 1; }
  }

  if(m.mode === "aggressive"){
    probs[4] += 4; probs[6] += 4; probs[1] -= 5; probs["W"] += 2;
  } else if(m.mode === "defensive"){
    probs[1] += 4; probs[2] += 2; probs[4] -= 3; probs[6] -= 3; probs["W"] -= 1; probs[0] += 1;
  }

  Object.keys(probs).forEach(k => { if(probs[k] < 0) probs[k] = 0; });
...
function simulateAIChase(target, oversLimit){
  const bowlers = state.userXI
    .map(idx => SQUADS[state.userTeam][idx])
    .filter(p => p.role === "bowler" || p.role === "allrounder")
    .map(p => ({
      name:p.name,
      rating:p.rating,
  ...
  let overRuns = [], wormPoints = [];
  let currentOver = 0;

  while(balls < oversLimit * 6 && wickets < 10 && score < target){
    const over = Math.floor(balls / 6) + 1;
    const phase = over <= Math.min(10, Math.floor(oversLimit * 0.2)) ? "Powerplay" : (over >= oversLimit - 5 ? "Death" : "Middle");
    const bowlerPool = bowlers.sort((a,b) => b.rating - a.rating);
    const bowler = phase === "Death" ? bowlerPool[0] : (phase === "Powerplay" ? bowlerPool[Math.min(1, bowlerPool.length - 1)] : bowlerPool[Math.min(2, bowlerPool.length - 1)]);
...
function finishMatch(){
  ...
  if(m.inningsMode === "chase"){
    ...
    if(chased){
      ...
      xpGain = 35 + Math.max(0, (10 - m.wickets) * 2);
      coinGain = 25 + Math.max(0, (10 - m.wickets));
  ...
  if(state.playerTournamentStats[pomKey]) state.playerTournamentStats[pomKey].awards += 1;

  const milestoneBonus = topScore() >= 50 ? 10 : 0;
  xpGain += milestoneBonus;
  coinGain += Math.floor(m.extras / 2);
  addProfileRewards(xpGain, coinGain);
...
function addProfileRewards(xpGain, coinGain){
  state.profile.xp += xpGain;
  state.profile.coins += coinGain;
  while(state.profile.xp >= state.profile.level * 100){
...
function showMatchResult(){
  ...
  els.resultRewards.innerHTML = `
    <div class="result-box"><strong>Rewards</strong><br>XP: ${state.profile.xp}<br>Coins: ${state.profile.coins}<br>Win streak: ${state.profile.winStreak}</div>
  `;
...
function awardPoints(winner, loser, winnerRuns, winnerBalls, loserRuns, loserBalls){
  const w = state.points[winner], l = state.points[loser];
  w.played += 1; w.won += 1; w.points += 2; w.runsFor += winnerRuns; w.ballsFor += winnerBalls; w.runsAgainst += loserRuns; w.ballsAgainst += loserBalls;
  l.played += 1; l.lost += 1; l.runsFor += loserRuns; l.ballsFor += loserBalls; l.runsAgainst += winnerRuns; l.ballsAgainst += winnerBalls;
  updateNRR(winner); updateNRR(loser);
}
...
function handleKnockoutProgress(fixture){
  ...
  if(fixture.stage.startsWith("Semi Final")){
    ...
    state.knockout[currentSemiKey] = { ...state.knockout[currentSemiKey], winner: fixture.winner, resultText: fixture.resultText, played: true, stats: fixture.stats };
    ...
    if(userWon){
      ...
      const finalSim = simulateKnockout({ id:"final", stage:"Final", teamA:state.knockout.semi1.winner, teamB:state.knockout.semi2.winner });
    ...
    state.knockout.final = { ...state.knockout.final, winner: fixture.winner, resultText: fixture.resultText, played:true, stats: fixture.stats };
...
function maybeComputePlayerOfTournament(){
  const sorted = Object.values(state.playerTournamentStats).sort((a,b) => (b.runs + b.awards * 20) - (a.runs + a.awards * 20));
...
function autoSimAllKnockouts(){
  ...
  state.knockout.final = simulateKnockout({ id:"final", stage:"Final", teamA:state.knockout.semi1.winner, teamB:state.knockout.semi2.winner });
...
function syncUIFromLoadedState(){
  ...
  if(state.started && state.schedule[state.currentMatchIndex]){
    prepareMatch(state.schedule[state.currentMatchIndex]);
  }
}
